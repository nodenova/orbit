...
be correct on inspection and gated by the isolation test in `orbit.eval.isolation`,
...
from orbit.attest.hashing import hash_artefact
from orbit.backends import mlx_kv
from orbit.backends.base import Backend, BackendUnavailable, Delta, ToolCallRenderer
from orbit.types import GenRequest, GenResult, KVState, StopReason, Usage
...
ACTIVE_ADAPTER: contextvars.ContextVar[str | None] = contextvars.ContextVar(
    "orbit_active_adapter", default=None
)
    ...
    def _logits_processors(self, req: GenRequest) -> list[Any]:
        ...
        **This is the half that used to be missing, and its absence was silent.**
        ...
        if req.json_schema is None or self._constrain_vocab_failed:
            return []
        from orbit.gateway.toolcall.constrain import Constrainer, logit_width_bound
