...
from orbit.backends.base import Backend, BackendUnavailable, Delta, render_default
from orbit.backends.mock import Fault, MockBackend
from orbit.backends.remote_tier1 import RUNG as REMOTE_RUNG
from orbit.backends.remote_tier1 import (
    RemoteConsentMissing,
    RemoteTier1Backend,
    check_consent,
)
from orbit.backends.resident_swap import RUNG as RESIDENT_SWAP_RUNG
from orbit.backends.resident_swap import (
    Occupant,
    ResidencySwitch,
    ResidentSwapBackend,
    SwapGuard,
)
from orbit.backends.second_opinion import RUNG as SECOND_OPINION_RUNG
from orbit.backends.second_opinion import SecondOpinionBackend
from orbit.config import Config
from orbit.offline import is_loopback_endpoint
...
def build_tier1(cfg: Config, tier0: Backend | None = None) -> Backend | None:
    ...
    from orbit.backends.mlx_tier1 import OptiqTier1Backend
...
def _require_loopback_endpoint(cfg: Config) -> None:
    ...
    `tier1.endpoint` said. `ORBIT_TIER1_ENDPOINT=https://collector.example/v1`
    was enough to ship candidate patches and the repository context they were
    generated from to a third party, with `orbit doctor` still reporting
    `offline.ok: true`.
    ...
    raise ValueError(
        ...
        "also makes `orbit doctor` report the offline claim as broken (sec 8.6)."
    )
...
def _endpoint_label(endpoint: str) -> str:
    ...
    where verdicts went, and it ends up in `stats()` and in `orbit doctor`."""
