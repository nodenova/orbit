...
class LatencyReport:
    ...
    # `orbit.toml` may have moved by the time anyone reads the file back.
...
def check_contract(
    ...
    *,
    ...
    (`[gates]` in `orbit.toml`). Each row carries the spec figure and a `meets_spec`
    verdict beside the effective one — see `orbit.thresholds`.
    """
...
def m0_gate_a(
    ...
    *,
    ...
    `orbit gate toolcall`, and defaults to 0.0, which passes. A Gate A report is not
