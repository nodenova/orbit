...
def _reject_option(value: str, what: str) -> str:
    ...
    `base_rev` arriving from `orbit.toml` is one hyphen away from writing a file of
...
class WorktreeRunner:
    ...
    def __init__(
        ...
        *,
        ...
        # Resolved against orbit's working directory, never against the repo under
        ...
        # against whatever the process's cwd happens to be, and for `orbit eval
    ...
    def _safe_scratch(self, scratch_dir: str | Path | None) -> Path:
        ...
        default = Path(tempfile.gettempdir()) / "orbit-worktrees"
    ...
    def _evaluate(self, diff: str | None, base_rev: str) -> PatchOutcome:
        ...
        work = self.scratch / f"orbit-{uuid.uuid4().hex[:12]}"
    ...
    def _remove_worktree_entry(self, work: Path) -> None:
        ...
        is how "prune orbit's leftover" is expressed without touching an entry
        orbit did not create.
        """
    ...
    def _apply(self, work: Path, patch_file: Path) -> tuple[bool, str]:
        ...
        for extra in ([], ["--3way"], ["-p0"]):
            ...
            # structurally unable to become one. The name is orbit's own today; the
