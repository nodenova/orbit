...
DEFAULT_CONFIG_PATH = Path(os.environ.get("ORBIT_CONFIG", "orbit.toml"))
...
class Tier0Config:
    ...
    # `orbit.toml` derives; the constant is the 30 recurrent layers, whose state
...
class EvalConfig:
    ...
    worktree_dir: str = "~/.cache/orbit/worktrees"
...
class GatesConfig:
    ...
    (`orbit.thresholds`), so a green gate on a relaxed floor still says how far
    ...
    # sec 7.3 chat contract, reported by `orbit bench latency` beside Gate A.
...
_ENV_MAP: dict[str, tuple[str, ...]] = {
    ...
    "ORBIT_PORT": ("server", "port"),
    "ORBIT_HOST": ("server", "host"),
    "ORBIT_API_KEY": ("server", "api_key"),
    "ORBIT_TIER0_MODEL": ("tier0", "model"),
    "ORBIT_TIER0_CONTAINER": ("tier0", "container_path"),
    "ORBIT_ADAPTER_DIR": ("tier0", "adapter_dir"),
    "ORBIT_TIER1_ENABLED": ("tier1", "enabled"),
    "ORBIT_TIER1_ENDPOINT": ("tier1", "endpoint"),
    "ORBIT_CANDIDATES": ("router", "candidates"),
    "ORBIT_NO_COMPACT": ("compaction", "enabled"),
    "ORBIT_AUDIT_LOG": ("attest", "audit_log"),
}
...
def _apply_env(cfg: Config) -> None:
    for env, path in _ENV_MAP.items():
        ...
        if isinstance(current, bool) or (current is None and env.endswith("_ENABLED")):
            ...
            value = float(raw)
        # ORBIT_NO_COMPACT=1 must *disable* compaction; the flag is negated.
        if env == "ORBIT_NO_COMPACT":
