...
from orbit.attest.audit import AuditLog, AuditRecord, now, sha256_text
from orbit.attest.receipt import Receipt, Tier0Attestation, Tier1Attestation
from orbit.backends.base import Backend, Delta, render_default
from orbit.backends.resident_swap import ResidentSwapBackend
from orbit.config import Config
from orbit.eval.worktree import WorktreeRunner
from orbit.eval.worktree import from_config as build_worktree_runner
...
from orbit.gateway.toolcall.repair import looks_like_tool_intent, repair
from orbit.gateway.toolcall.replay import ReplayMap, render_call
from orbit.router.cascade import Cascade, CascadeInfo
from orbit.router.classify import Classification, classify
...
# Enough to cover a working session's recent history for `/orbit/trace/last` and
...
class Pipeline:
    def __init__(
        ...
        *,
        ...
        # Bounded. Only `/orbit/trace/last` and the measurement discipline read
        ...
        # Rolling tool-call tally, so `orbit gate toolcall` can report the sec 10.2
        ...
        return {
            ...
            "escalation": (
                ...
                if self.worktree is not None
                    ...
                    "enabled": False,
                    "reason": "no test command configured (orbit.toml [eval])",
                }
            ),
