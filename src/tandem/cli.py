"""`tandem` — the command line.

Subcommands map onto the spec's milestones, so a user working through M0-M6 can run
each gate by name:

    tandem serve                      sec 8   gateway, three wire protocols
    tandem doctor                     sec 8.6 offline posture + runtime status
    tandem extract a0|a1|a2           sec 6   build an adapter corpus
    tandem profile                    sec 6.4 routing profile
    tandem train sft|dpo              sec 6   train an adapter
    tandem eval merge                 sec 10.1 the merge eval, four bars
    tandem gate toolcall|isolation    sec 10.2, 4.2
    tandem bench latency|tier1        sec 10.4, M0 Gate A/B
    tandem audit verify               sec 9.2
"""
...
def cmd_eval_merge(args: argparse.Namespace) -> int:
    ...
    # measurable at all; without a `[eval]` block in tandem.toml it is None and
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="tandem", description=__doc__.splitlines()[0])
    p.add_argument("--config", default=None, help="path to tandem.toml")
    ...
    e.add_argument(
        "--no-worktree",
        action="store_true",
        help="skip the test and lint metrics even when tandem.toml [eval] configures them",
    )
